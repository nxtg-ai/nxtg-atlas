...
def _portfolio_summary(portfolio: Portfolio) -> list[str]:
    ...
    # Compression libs
    has_compress_md = sum(1 for p in projects if p.tech_stack.compression_libs)
    if has_compress_md:
        compress_counter_md: Counter[str] = Counter()
        for p in projects:
            for cl in p.tech_stack.compression_libs:
                compress_counter_md[cl] += 1
        compress_names = ", ".join(f"{t} ({c})" for t, c in compress_counter_md.most_common(8))
        lines.append(f"**Compression Libs**: {has_compress_md}/{n} projects · {compress_names}")

    # Email libs
    has_email_md = sum(1 for p in projects if p.tech_stack.email_libs)
    if has_email_md:
        email_counter_md: Counter[str] = Counter()
        for p in projects:
            for el in p.tech_stack.email_libs:
                email_counter_md[el] += 1
        email_names = ", ".join(f"{t} ({c})" for t, c in email_counter_md.most_common(8))
        lines.append(f"**Email Libs**: {has_email_md}/{n} projects · {email_names}")

    # A11y tools
    has_a11y_md = sum(1 for p in projects if p.tech_stack.a11y_tools)
    if has_a11y_md:
        a11y_counter_md: Counter[str] = Counter()
        for p in projects:
            for at in p.tech_stack.a11y_tools:
                a11y_counter_md[at] += 1
    ...
    has_desktop_md = sum(1 for p in projects if p.tech_stack.desktop_frameworks)
    if has_desktop_md:
        desktop_counter_md: Counter[str] = Counter()
        for p in projects:
            for df in p.tech_stack.desktop_frameworks:
                desktop_counter_md[df] += 1
    ...
    has_anim_md = sum(1 for p in projects if p.tech_stack.animation_libs)
    if has_anim_md:
        anim_counter_md: Counter[str] = Counter()
        for p in projects:
            for al in p.tech_stack.animation_libs:
                anim_counter_md[al] += 1
        anim_names = ", ".join(f"{t} ({c})" for t, c in anim_counter_md.most_common(8))
        lines.append(f"**Animation Libs**: {has_anim_md}/{n} projects · {anim_names}")

    has_routing_md = sum(1 for p in projects if p.tech_stack.routing_libs)
    if has_routing_md:
        routing_counter_md: Counter[str] = Counter()
        for p in projects:
            for rl in p.tech_stack.routing_libs:
                routing_counter_md[rl] += 1
    ...
    has_rl_md = sum(1 for p in projects if p.tech_stack.rate_limiters)
    if has_rl_md:
        rl_counter_md: Counter[str] = Counter()
        for p in projects:
            for rl in p.tech_stack.rate_limiters:
                rl_counter_md[rl] += 1
        rl_names = ", ".join(f"{t} ({c})" for t, c in rl_counter_md.most_common(8))
        lines.append(f"**Rate Limiters**: {has_rl_md}/{n} projects · {rl_names}")

    has_mig_md = sum(1 for p in projects if p.tech_stack.db_migration_tools)
    if has_mig_md:
        mig_counter_md: Counter[str] = Counter()
        for p in projects:
    ...
    has_ana_md = sum(1 for p in projects if p.tech_stack.analytics_tools)
    if has_ana_md:
        ana_counter_md: Counter[str] = Counter()
        for p in projects:
            for a in p.tech_stack.analytics_tools:
                ana_counter_md[a] += 1
        ana_names = ", ".join(f"{t} ({c})" for t, c in ana_counter_md.most_common(8))
        lines.append(f"**Analytics**: {has_ana_md}/{n} projects · {ana_names}")

    has_mob_md = sum(1 for p in projects if p.tech_stack.mobile_frameworks)
    if has_mob_md:
        mob_counter_md: Counter[str] = Counter()
        for p in projects:
...
def _project_details(projects: list[Project]) -> list[str]:
    ...
    for p in projects:
        ...
        if p.tech_stack.compression_libs:
            lines.append(f"- **Compression**: {', '.join(p.tech_stack.compression_libs[:8])}")

        if p.tech_stack.email_libs:
            lines.append(f"- **Email**: {', '.join(p.tech_stack.email_libs[:8])}")
        ...
        if p.tech_stack.animation_libs:
            lines.append(f"- **Animation**: {', '.join(p.tech_stack.animation_libs[:8])}")
        ...
        if p.tech_stack.rate_limiters:
            lines.append(f"- **Rate Limiting**: {', '.join(p.tech_stack.rate_limiters[:8])}")
        ...
        if p.tech_stack.analytics_tools:
            lines.append(f"- **Analytics**: {', '.join(p.tech_stack.analytics_tools[:8])}")
...
def _json_portfolio_summary(projects: list[Project]) -> dict:
    ...
    # Compression libs
    compress_counter_j: Counter[str] = Counter()
    for p in projects:
        for cl in p.tech_stack.compression_libs:
            compress_counter_j[cl] += 1
    has_compress_j = sum(1 for p in projects if p.tech_stack.compression_libs)

    # Email libs
    email_counter_j: Counter[str] = Counter()
    for p in projects:
        for el in p.tech_stack.email_libs:
            email_counter_j[el] += 1
    has_email_j = sum(1 for p in projects if p.tech_stack.email_libs)

    # A11y tools
    a11y_counter_j: Counter[str] = Counter()
    for p in projects:
        for at in p.tech_stack.a11y_tools:
            a11y_counter_j[at] += 1
    ...
    # Desktop frameworks
    desktop_counter_j: Counter[str] = Counter()
    for p in projects:
        for df in p.tech_stack.desktop_frameworks:
    ...
    # Animation libs
    anim_counter_j: Counter[str] = Counter()
    for p in projects:
        for al in p.tech_stack.animation_libs:
            anim_counter_j[al] += 1
    has_anim_j = sum(1 for p in projects if p.tech_stack.animation_libs)

    # Routing libs
    routing_counter_j: Counter[str] = Counter()
    for p in projects:
        for rl in p.tech_stack.routing_libs:
    ...
    # Rate limiters
    rl_counter_j: Counter[str] = Counter()
    for p in projects:
        for rl in p.tech_stack.rate_limiters:
            rl_counter_j[rl] += 1
    has_rl_j = sum(1 for p in projects if p.tech_stack.rate_limiters)

    # DB migration tools
    mig_counter_j: Counter[str] = Counter()
    for p in projects:
        for mt in p.tech_stack.db_migration_tools:
            mig_counter_j[mt] += 1
    ...
    # Analytics tools
    ana_counter_j: Counter[str] = Counter()
    for p in projects:
        for a in p.tech_stack.analytics_tools:
            ana_counter_j[a] += 1
    has_ana_j = sum(1 for p in projects if p.tech_stack.analytics_tools)

    # Mobile frameworks
    mob_counter_j: Counter[str] = Counter()
    for p in projects:
        for m in p.tech_stack.mobile_frameworks:
            mob_counter_j[m] += 1
