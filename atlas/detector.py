...
def detect_math_libs(project_path: Path) -> list[str]:
    """Detect math and scientific computing libraries."""
    tools: list[str] = []
    seen: set[str] = set()
    ...
    go_map = {
        "gonum.org/v1/gonum": "Gonum",
        "montanaflynn/stats": "stats",
        "james-bowman/sparse": "sparse",
    }
    for dep, name in go_map.items():
        if dep.lower() in go_content:
            _add(name)
    ...
    # Java
    java_deps_math: list[str] = []
    for build_file in ("build.gradle", "build.gradle.kts", "pom.xml"):
        bf = project_path / build_file
        if bf.exists():
            try:
                java_deps_math.append(bf.read_text())
            except Exception:
                pass
    java_content_math = " ".join(java_deps_math)
    ...
    for dep, name in java_map_math.items():
        if dep in java_content_math:
            _add(name)

    return sorted(tools)


def detect_async_libs(project_path: Path) -> list[str]:
    """Detect concurrency and async libraries."""
    tools: list[str] = []
    seen: set[str] = set()
    ...
    go_map = {
        "golang.org/x/sync": "x/sync",
        "sourcegraph/conc": "conc",
        "panjf2000/ants": "ants",
        "alitto/pond": "pond",
    }
    for dep, name in go_map.items():
    ...
    # Java
    java_deps_async: list[str] = []
    for build_file in ("build.gradle", "build.gradle.kts", "pom.xml"):
        bf = project_path / build_file
        if bf.exists():
            try:
                java_deps_async.append(bf.read_text())
            except Exception:
                pass
    java_content_async = " ".join(java_deps_async)
    ...
    for dep, name in java_map_async.items():
        if dep in java_content_async:
            _add(name)

    return sorted(tools)


def detect_compression_libs(project_path: Path) -> list[str]:
    """Detect compression and archive libraries."""
    tools: list[str] = []
    seen: set[str] = set()
    ...
    go_map = {
        ...
        "klauspost/pgzip": "pgzip",
        "pierrec/lz4": "LZ4",
        "datadog/zstd": "Zstandard",
        "mholt/archiver": "archiver",
        "ulikunitz/xz": "xz",
    }
    for dep, name in go_map.items():
        if dep.lower() in go_content:
    ...
    # Java
    java_deps_compress: list[str] = []
    for build_file in ("build.gradle", "build.gradle.kts", "pom.xml"):
        bf = project_path / build_file
        if bf.exists():
            try:
                java_deps_compress.append(bf.read_text())
            except Exception:
                pass
    java_content_compress = " ".join(java_deps_compress)
    ...
    for dep, name in java_map_compress.items():
        if dep in java_content_compress:
            _add(name)
