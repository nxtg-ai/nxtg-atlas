...
class TestConnections:
    ...
    def test_connections_type_list_shows_count(self, portfolio_dir, tmp_path):
        ...
        result = runner.invoke(app, ["connections", "--type", "list"])
        assert result.exit_code == 0
        assert "77 categories" in result.output
...
class TestSearch:
    ...
    def test_search_limit_json(self, portfolio_dir, tmp_path):
        runner.invoke(app, ["init"])
        for i in range(5):
            d = tmp_path / f"proj{i}"
            d.mkdir()
            ...
            with patch("atlas.cli.scan_project", return_value=proj):
                runner.invoke(app, ["add", str(d)])
        result = runner.invoke(app, ["search", "python", "--format", "json", "--limit", "2"])
        assert result.exit_code == 0
        data = json.loads(result.output)
        assert len(data["projects"]) == 2

    def test_search_limit_rich(self, portfolio_dir, tmp_path):
        runner.invoke(app, ["init"])
        for i in range(5):
            d = tmp_path / f"proj{i}"
            d.mkdir()
            ...
            with patch("atlas.cli.scan_project", return_value=proj):
                runner.invoke(app, ["add", str(d)])
        result = runner.invoke(app, ["search", "python", "--limit", "2"])
        assert result.exit_code == 0
        assert "2 project" in result.output

    def test_search_limit_with_sort(self, portfolio_dir, tmp_path):
        runner.invoke(app, ["init"])
        for name, loc in [("a", 100), ("b", 5000), ("c", 1000), ("d", 3000)]:
            ...
            with patch("atlas.cli.scan_project", return_value=proj):
                runner.invoke(app, ["add", str(d)])
        result = runner.invoke(app, ["search", "python", "--format", "json", "--sort", "loc", "--limit", "2"])
        assert result.exit_code == 0
        data = json.loads(result.output)
        assert len(data["projects"]) == 2
